...
# The first check runs one interval after start, so check every 5s during the start period.
HEALTHCHECK --interval=1m --timeout=5s --start-period=1m --start-interval=5s \
